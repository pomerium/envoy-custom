...
if [[ $# -gt 0 ]]; then
    ...
    all_tests=$(bazel query 'kind(cc_.*, tests(//...))')
    COVERAGE_TARGETS=()
...
COVERAGE_VALUE="$(genhtml --ignore-errors inconsistent --prefix "${PWD}" --output "${COVERAGE_DIR}" "${COVERAGE_DATA}" | tee /dev/stderr | grep lines... | cut -d ' ' -f 4)"
