...
class StreamTrackerShutdownExit : public testing::Test, public Envoy::Event::TestUsingSimulatedTime {
public:
  void SetUp() override {
    api_ = Api::createApiForTest();
    dispatcher_ = api_->allocateDispatcher("test_thread");
    ...
    ON_CALL(context_.lifecycle_notifier_,
            registerCallback(
              Envoy::Server::ServerLifecycleNotifier::Stage::ShutdownExit,
              testing::An<Envoy::Server::ServerLifecycleNotifier::StageCallbackWithCompletion>()))
      .WillByDefault(
        [this](auto, Envoy::Server::ServerLifecycleNotifier::StageCallbackWithCompletion cb)
          -> Envoy::Server::ServerLifecycleNotifier::HandlePtr {
        shutdown_exit_cb_ = std::move(cb);
        return nullptr;
      });

    ON_CALL(context_.drain_manager_, addOnDrainCloseCb)
      .WillByDefault(
        [this](Network::DrainDirection, Network::DrainDecision::DrainCloseCb cb)
  ...
  std::pair<Envoy::Event::PostCb, std::weak_ptr<void>> makeGuard() {
    auto witness = std::make_shared<bool>();
    std::weak_ptr<void> wp = witness;
    return {[witness = std::move(witness)] {}, wp};
  }
  ...
  Envoy::Server::ServerLifecycleNotifier::StageCallbackWithCompletion shutdown_exit_cb_;
  Network::DrainDecision::DrainCloseCb drain_cb_;
};

TEST_F(StreamTrackerShutdownExit, ShutdownExitCalledTwice_DoesNotHang) {
  ASSERT_NE(nullptr, shutdown_exit_cb_);

  auto [guard1, guard1_alive] = makeGuard();
  shutdown_exit_cb_(std::move(guard1));

  auto [guard2, guard2_alive] = makeGuard();
  shutdown_exit_cb_(std::move(guard2));

  dispatcher_->run(Envoy::Event::Dispatcher::RunType::Block);

  EXPECT_TRUE(guard1_alive.expired());
  EXPECT_TRUE(guard2_alive.expired());
}

TEST_F(StreamTrackerShutdownExit, DrainThenShutdownExit_DoesNotHang) {
  ASSERT_NE(nullptr, drain_cb_);
  ASSERT_NE(nullptr, shutdown_exit_cb_);

  ASSERT_OK(drain_cb_(std::chrono::milliseconds{0}));
  auto [guard, guard_alive] = makeGuard();
  shutdown_exit_cb_(std::move(guard));

  dispatcher_->run(Envoy::Event::Dispatcher::RunType::Block);
  EXPECT_TRUE(guard_alive.expired());
}

TEST_F(StreamTrackerShutdownExit, ShutdownExitAfterDrainComplete_DoesNotHang) {
  ASSERT_NE(nullptr, drain_cb_);
  ASSERT_NE(nullptr, shutdown_exit_cb_);
